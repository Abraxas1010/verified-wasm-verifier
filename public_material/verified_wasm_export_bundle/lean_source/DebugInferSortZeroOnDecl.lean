...
namespace HeytingLean.DebugInferSortZeroOnDecl
...
private def runDecl (declName : Name) : IO Json := do
  ...
  | some ci =>
      match (← mkSurfaces env ci.type) with
      ...
      | .ok (targetExpr, envData, natTy, stringTy) =>
          let some envC := EnvironmentSKY.envCombWithMode? .classic [] envData
            | pure <| Json.mkObj [("error", Json.str "failed to compile environment")]
          let some fullC := FullKernelSKY.compileFullWithMode? .classic
            | pure <| Json.mkObj [("error", Json.str "failed to compile full kernel bundle")]
          let some fuelC := FullKernelSKY.encodeNatCombWithMode? .classic 40
            | pure <| Json.mkObj [("error", Json.str "failed to compile fuel")]
          let some targetC := FullKernelSKY.compileExprNatUnitWithMode? .classic targetExpr
            | pure <| Json.mkObj [("error", Json.str "failed to compile target expr")]
          ...
          | .error err, _ => pure <| Json.mkObj [("error", Json.str err)]
          | _, .error err => pure <| Json.mkObj [("error", Json.str err)]
...
def main (args : List String) : IO UInt32 := do
  ...
  | [decl] =>
      ...
      pure 0
  | _ =>
      IO.eprintln "usage: lake env lean --run DebugInferSortZeroOnDecl.lean <Decl.Name>"
      pure 1

end HeytingLean.DebugInferSortZeroOnDecl

def main (args : List String) : IO UInt32 :=
  HeytingLean.DebugInferSortZeroOnDecl.main args
