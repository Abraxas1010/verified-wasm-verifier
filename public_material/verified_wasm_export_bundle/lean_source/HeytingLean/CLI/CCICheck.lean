...
Reads a certificate JSON file, decodes, runs the checker, and exits 0/1.
-/
...
def usage : String := "usage: cci_check [PATH] [--cert PATH]  (env fallback: CCI_CERT_PATH)"
...
def main (argv : List String) : IO Unit := HeytingLean.CLI.CCICheck.run argv
