...
/-!
# `minic_cosmwasm_export` (MiniC → CosmWasm WAT/WASM)

Produce a CosmWasm-ABI-compatible WASM image for smoke-level integration:
- exports `instantiate/execute/query` (region-pointer ABI),
- exports `allocate/deallocate` + `interface_version_8`,
- exports `memory`.

Important:
- The verified core remains: MiniC → WasmMini compilation + correctness proofs.
- The CosmWasm adapter is currently **unverified glue** (see `WasmMini.emitCosmWasmModule`).

Outputs:
- `kernel_cosmwasm.wat`  (CosmWasm-compatible module)
- `kernel_cosmwasm.wasm` (assembled, if `wat2wasm` exists)
- Optional validation via `wasm-validate`.
-/

namespace HeytingLean
namespace CLI
namespace MiniCCosmWasmExport
...
structure Config where
  outDir : String := "dist/minic_cosmwasm_export"
  assemble : Bool := true
deriving Repr

private def usage : String :=
  String.intercalate "\n"
    [ "minic_cosmwasm_export"
    , ""
    , "Emits:"
    , "  - kernel_cosmwasm.wat        (CosmWasm-compatible WAT)"
    , "  - kernel_cosmwasm.wasm       (assembled if `wat2wasm` exists)"
    , ""
    , "Flags:"
...
def main (argv : List String) : IO UInt32 := do
  try
    ...
    let watPath := outDir / "kernel_cosmwasm.wat"
    let wasmPath := outDir / "kernel_cosmwasm.wasm"
    ...
    let funsWasm : List HeytingLean.WasmMini.Fun :=
      ([add1Minic, add2Minic, ifDemoMinic, whileSumMinic] : List MiniC.Fun).map
        HeytingLean.MiniC.ToWasmMini.compileFun

    let wat := HeytingLean.WasmMini.emitCosmWasmModule funsWasm
    writeFile watPath wat
    IO.println s!"[minic_cosmwasm_export] wrote {watPath}"
    ...
    | .error e =>
        IO.eprintln s!"[minic_cosmwasm_export] assemble skipped/failed: {e}"
        return (0 : UInt32)
    | .ok () =>
        match ← validateWasm wasmPath with
        | .ok () =>
            IO.println s!"[minic_cosmwasm_export] wrote {wasmPath}"
            return (0 : UInt32)
        | .error e =>
            IO.eprintln s!"[minic_cosmwasm_export] wasm validation failed: {e}"
            return (1 : UInt32)
  catch e =>
    IO.eprintln s!"[minic_cosmwasm_export] error: {e.toString}"
    return (1 : UInt32)

end MiniCCosmWasmExport
end CLI
end HeytingLean

def main (args : List String) : IO UInt32 :=
  HeytingLean.CLI.MiniCCosmWasmExport.main args
