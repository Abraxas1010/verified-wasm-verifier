...
lemma G2_eq_tsum_G2Term (z : ℍ) : G2 z = ∑' m, ∑' n, G2Term z ![m, n] := by
  ...
  · rw [← tsum_eq_of_summable_unconditional (L := symmetricIcc ℤ)]
    · congr with a
      rw [e2Summand, tsum_eq_of_summable_unconditional_symmetricIco
        (hf := summable_right_one_div_linear_sub_one_div_linear_succ z a), ← Summable.tsum_add
  ...
  · simpa using (G2Term_prod_summable z).prod
...
lemma G2_S_action_eq_tsum_G2Term (z : ℍ) : ((z : ℂ) ^ 2)⁻¹ * G2 (S • z) - -2 * π * I / z =
  ...
  rw [← tsum_symmetricIco_tsum_sub_eq z, ← tsum_symmetricIco_tsum_eq_S_act z,
    ← tsum_eq_of_summable_unconditional_symmetricIco, ← Summable.tsum_sub]
...
lemma G2_slash_action (γ : SL(2, ℤ)) : G2 ∣[(2 : ℤ)] γ = G2 - D2 γ := by
  ...
  | mem g hg =>
      ...
      · ext z
        simp only [Pi.sub_apply, h1, ModularForm.SL_slash_apply, G2_S_transform z,
          ModularGroup.S, denom_apply, pow_two]
        simp [D2, ModularGroup.denom_apply]
        set A : ℂ :=
          G2
            (Matrix.SpecialLinearGroup.toGL
                ((Matrix.SpecialLinearGroup.map (Int.castRingHom ℝ)) ⟨!![0, -1; 1, 0], S._proof_1⟩) •
              z)
        change A * ((z : ℂ) ^ 2)⁻¹ = ((z : ℂ)⁻¹) * ((z : ℂ)⁻¹) * A -
          -(2 * ↑π * I) / (z : ℂ) - 2 * ↑π * I / (z : ℂ)
        have hpow : ((↑z : ℂ) ^ 2)⁻¹ = ((↑z : ℂ)⁻¹) ^ 2 := by
          exact (inv_pow (↑z : ℂ) 2).symm
        calc
          A * ((z : ℂ) ^ 2)⁻¹ = A * (((z : ℂ)⁻¹) ^ 2) := by exact congrArg (fun t : ℂ => A * t) hpow
          _ = ((z : ℂ)⁻¹) * ((z : ℂ)⁻¹) * A - -(2 * ↑π * I) / (z : ℂ) - 2 * ↑π * I / (z : ℂ) := by ring
  ...
  | mul g h _ _ ig ih =>
      ...
      ext w
      simp [SlashAction.neg_slash, SL_slash, sub_eq_add_neg, add_assoc, add_comm]
  | inv g _ ig =>
