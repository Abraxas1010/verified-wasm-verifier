...
/-!
# EigenformSoup SUBLEQ-lane trace parity bundle generator

Emits a C bundle for runtime parity checks over a deterministic SUBLEQ-like
control-lane payload used for LES comparison studies.
-/

namespace HeytingLean.CLI.EigenformSoupSubleqTraceParityMain

open System

private def usage : String :=
  String.intercalate "\n"
    [ "eigenform_soup_subleq_trace_parity"
    , "  Generates a runtime parity C bundle for SUBLEQ-lane trace payloads."
    , ""
    , "Options:"
    ...
    , "  --out-dir <path> | --out-dir=<path>    output directory (default: dist/eigenform_soup_subleq_trace_parity)"
...
private def parseOutDir (argv : List String) : FilePath :=
  let raw := (findOptValue argv "--out-dir").getD "dist/eigenform_soup_subleq_trace_parity"
  FilePath.mk raw

private def criticalSize (depth : Nat) : Nat :=
  Nat.shiftLeft 1 (depth + 7)

private def growthRate (size depth : Nat) : Nat :=
  if size < criticalSize depth then 0 else (size - criticalSize depth + 1) / 4

private def firstEigenformEpoch? (size depth : Nat) : Option Nat :=
  let rate := growthRate size depth
  if rate = 0 then none else
    let base := criticalSize depth / (Nat.max 1 rate)
    some (Nat.max 64 base)
...
private def traceJson (size depth epochs sampleEvery : Nat) : String :=
  let runtimeModel := "subleq-proxy-v1"
  ...
  let theoreticalMinSize := Nat.shiftLeft 1 (depth + 2)
...
private def kernelSource (jsonPayload : String) : String :=
  String.intercalate "\n"
    [ "/* EigenformSoup generated SUBLEQ-lane C trace artifact */"
...
def main (argv0 : List String) : IO UInt32 := do
  ...
  | .error err, _, _, _ =>
      IO.eprintln s!"eigenform_soup_subleq_trace_parity: {err}"
  ...
  | .ok size, .ok depth, .ok epochs, .ok sampleEvery =>
      ...
      try
        ...
        IO.println s!"eigenform_soup_subleq_trace_parity: wrote bundle"
        ...
        IO.eprintln s!"eigenform_soup_subleq_trace_parity: failed: {e}"
        return 1

end HeytingLean.CLI.EigenformSoupSubleqTraceParityMain

open HeytingLean.CLI.EigenformSoupSubleqTraceParityMain in
def main (args : List String) : IO UInt32 :=
  HeytingLean.CLI.EigenformSoupSubleqTraceParityMain.main args
