...
import HeytingLean.Crypto.QKD.E91.DI.Tsirelson.QuantumCorrelations
...
private theorem norm_sum_le_two_sqrt_two :
  ...
  have hsq : (norm (S.b0 + S.b1) + norm (S.b0 - S.b1)) ^ 2 <= (8 : Real) := by
    have hadd :=
      add_sq_le (a := norm (S.b0 + S.b1)) (b := norm (S.b0 - S.b1))
...
theorem tsirelson_bound :
  ...
  calc
    ...
    _ <=
        norm S.a0 * norm (S.b0 + S.b1) + norm S.a1 * norm (S.b0 - S.b1) := by
          gcongr
