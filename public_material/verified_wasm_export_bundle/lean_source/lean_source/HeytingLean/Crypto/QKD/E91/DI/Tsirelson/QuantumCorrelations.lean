...
import HeytingLean.Crypto.QKD.E91.DI.CHSH.Correlations

/-!
# Quantum correlators (vector strategy model)

We model a "quantum" CHSH strategy by unit vectors `a0,a1,b0,b1` in a real inner product
space, with correlators

`E(x,y) = inner ℝ (a_x) (b_y)`.

This is a standard abstraction for Tsirelson-style arguments and lets us prove the
`2 * sqrt 2` upper bound without importing density matrices.
-/
