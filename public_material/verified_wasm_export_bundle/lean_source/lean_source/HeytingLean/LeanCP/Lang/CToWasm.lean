...
def translateFunc (fenv : FuncEnv) (f : CFunDecl) : WasmFunc :=
  ...
  { name := f.name
    ...
    body := translateStmt env fenv f.body ++
            -- Append unreachable after the body. All control paths hit `return`,
            -- so this is dead code, but it satisfies the Wasm type checker:
            -- without it, a function with (result i32) whose body ends in a
            -- void-typed `if` block fails validation.
            (if resultTypes.isEmpty then [] else [.unreachable]) }
